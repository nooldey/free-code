...
ART
  printf "${RESET}"
  printf "${DIM}  Uninstalling freecode${RESET}\n"
  echo ""
}
...
remove_install_dir() {
  if [ -d "$INSTALL_DIR" ]; then
    ...
    if [ "$INSTALL_DIR" != "$allowed_primary" ] && [ "$INSTALL_DIR" != "$allowed_macos" ]; then
      fail "安装目录不符合白名单规则（仅允许 \$HOME/$APP_NAME 或 \$HOME/.config/$APP_NAME），拒绝删除。"
    fi
